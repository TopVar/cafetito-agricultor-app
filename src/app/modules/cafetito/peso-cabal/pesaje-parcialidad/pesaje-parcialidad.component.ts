...
import { CafetitoService } from 'src/app/modules/componentes-comunes/servicios/cafetito.service';
...
export class PesajeParcialidadComponent implements OnInit {
  ...
  constructor(private cafetitoService: CafetitoService) { 
  ...
  ngOnInit(): void {
    this.inicio = true
    this.cafetitoService.getParcialidadesPesaje().subscribe(res =>{
      if(res.length != 0){
