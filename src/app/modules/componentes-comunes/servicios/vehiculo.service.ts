...
export class VehiculoService {    
  ...
  constructor(private http: HttpClient) { }
    ...
    autorizarVehiculo(placa: string): Observable<any> {
      const headers = new HttpHeaders().set('Authorization', `Bearer ${this.token}`);

      return this.http.post<any>(environment.BASE_WS_LOCAL + `/cafetito/vehiculos/autorizar/${placa}`, { headers, responseType: 'text' });
    }

    rechazarVehiculo(placa: string): Observable<any> {
      const headers = new HttpHeaders().set('Authorization', `Bearer ${this.token}`);

      return this.http.post<any>(environment.BASE_WS_LOCAL + `/cafetito/vehiculos/rechazar/${placa}`, { headers, responseType: 'text' });
    }
